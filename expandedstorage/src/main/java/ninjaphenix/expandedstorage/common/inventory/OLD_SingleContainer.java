...
public final class OLD_SingleContainer extends OLD_AbstractContainer<SingleScreenMeta>
{
    ...
    public OLD_SingleContainer(final int windowId, final BlockPos pos, final IInventory inventory, final PlayerEntity player,
            @Nullable final ITextComponent displayName)
    {
        super(OLD_ModContent.SINGLE_CONTAINER_TYPE, windowId, pos, inventory, player, getNearestSize(inventory.getSizeInventory()), displayName);
    ...
    public static class Factory implements IContainerFactory<OLD_SingleContainer>
    {
        @Nullable @Override
        public OLD_SingleContainer create(final int windowId, final PlayerInventory playerInventory, @Nullable final PacketBuffer data)
        {
            if (data != null)
            {
                final int inventorySize = data.readInt();
                final BlockPos pos = data.readBlockPos();
                return new OLD_SingleContainer(windowId, pos, new Inventory(inventorySize), playerInventory.player, null);
            }
            return null;
        }
    }
