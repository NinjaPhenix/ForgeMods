...
{
    private static final ImmutableSortedMap<Integer, PagedScreenMeta> SIZES = new ImmutableSortedMap.Builder<Integer, PagedScreenMeta>(Integer::compare).put(27, new PagedScreenMeta(9, 3, 1, 27, getTexture("shared", 9, 3), 208, 192)) // Wood
    ...
    public PagedContainer(final int windowId, final BlockPos pos, final IInventory inventory, final PlayerEntity player, final @Nullable ITextComponent displayName)
    {
        ...
        for (int i = 0; i < 9; i++)
        {
            addSlot(new Slot(player.inventory, i, left + 18 * i, top + 58));
        }
    }
    ...
    private static PagedScreenMeta getNearestSize(final int invSize)
    {
        ...
        if (val != null && largestKey > invSize && largestKey - invSize <= val.WIDTH)
        {
            return val;
        }
    ...
    public void resetSlotPositions(final boolean makeSlots)
    {
        for (int i = 0; i < INVENTORY.getSizeInventory(); i++)
        {
            ...
            if (y >= SCREEN_META.HEIGHT)
            {
                y = (18 * (y % SCREEN_META.HEIGHT)) - 2000;
            }
            else
            {
                y = y * 18;
            ...
            if (makeSlots)
            {
                addSlot(new Slot(INVENTORY, i, x * 18 + 8, y + 18));
            }
            else
            {
                inventorySlots.get(i).yPos = y + 18;
            }
        }
    }
    ...
    public void moveSlotRange(final int min, final int max, final int yChange)
    {
        ...
        {
            inventorySlots.get(i).yPos += yChange;
        }
    }
    ...
    {
        @Override
        public @Nullable PagedContainer create(final int windowId, final PlayerInventory playerInventory, final @Nullable PacketBuffer data)
        {
            if (data == null)
            {
                return null;
