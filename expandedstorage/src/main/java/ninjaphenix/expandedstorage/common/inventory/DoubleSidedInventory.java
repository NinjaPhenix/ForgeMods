...
public final class DoubleSidedInventory implements ISidedInventory
{
    ...
    public int[] getSlotsForFace(final Direction direction)
    {
        ...
        int index = 0;
        for (final int slot : firstSlots)
        {
            combined[index++] = slot;
        }
        for (final int slot : secondSlots)
        {
            combined[index++] = slot + first.getSizeInventory();
        }
        return combined;
    }

    @Override
    public boolean canInsertItem(final int slot, final ItemStack stack, final @Nullable Direction direction)
    {
        if (slot >= first.getSizeInventory())
        {
            return second.canInsertItem(slot - first.getSizeInventory(), stack, direction);
        }
    ...
    public boolean canExtractItem(final int slot, final ItemStack stack, final Direction direction)
    {
        if (slot >= first.getSizeInventory())
        {
            return second.canExtractItem(slot - first.getSizeInventory(), stack, direction);
        }
    ...
    {
        return first.isUsableByPlayer(player) && second.isUsableByPlayer(player);
    }
    ...
    public boolean isPart(final ISidedInventory inventory)
    {
    ...
    public ItemStack getStackInSlot(final int slot)
    {
        if (slot >= first.getSizeInventory())
        {
            return second.getStackInSlot(slot - first.getSizeInventory());
        }
    ...
    public ItemStack decrStackSize(final int slot, final int amount)
    {
        if (slot >= first.getSizeInventory())
        {
            return second.decrStackSize(slot - first.getSizeInventory(), amount);
        }
    ...
    public ItemStack removeStackFromSlot(final int slot)
    {
        if (slot >= first.getSizeInventory())
        {
            return second.removeStackFromSlot(slot - first.getSizeInventory());
        }
    ...
    public void setInventorySlotContents(final int slot, final ItemStack stack)
    {
        if (slot >= first.getSizeInventory())
        {
            second.setInventorySlotContents(slot - first.getSizeInventory(), stack);
        }
        else
        {
            first.setInventorySlotContents(slot, stack);
        }
    }
    ...
    public boolean isItemValidForSlot(final int slot, final ItemStack stack)
    {
        if (slot >= first.getSizeInventory())
        {
            return second.isItemValidForSlot(slot - first.getSizeInventory(), stack);
        }
