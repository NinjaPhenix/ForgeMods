...
{
    // @formatter:off
    private static final ImmutableSortedMap<Integer, ScrollableScreenMeta> SIZES = new ImmutableSortedMap.Builder<Integer, ScrollableScreenMeta>(Integer::compare).put(27, new ScrollableScreenMeta(9, 3, 27, getTexture("shared", 9, 3), 208, 192)) // Wood
    ...
    public ScrollableContainer(final int windowId, final BlockPos pos, final IInventory inventory, final PlayerEntity player, final @Nullable ITextComponent displayName)
    {
    ...
    public void setSlotRange(final int min, final int max, final IntUnaryOperator yPos)
    {
        for (int i = min; i < max; i++)
        {
            inventorySlots.get(i).yPos = yPos.applyAsInt(i);
        }
    }
    ...
    {
        @Override
        public @Nullable ScrollableContainer create(final int windowId, final PlayerInventory playerInventory, final @Nullable PacketBuffer data)
        {
            if (data == null)
            {
                return null;
