...
public final class OLD_ScrollableContainer extends OLD_AbstractContainer<ScrollableScreenMeta>
{
    // @formatter:off
    ...
    public OLD_ScrollableContainer(final int windowId, final BlockPos pos, final IInventory inventory, final PlayerEntity player,
            @Nullable final ITextComponent displayName)
    {
        super(OLD_ModContent.SCROLLABLE_CONTAINER_TYPE, windowId, pos, inventory, player, getNearestSize(inventory.getSizeInventory()), displayName);
    ...
    public static class Factory implements IContainerFactory<OLD_ScrollableContainer>
    {
        @Nullable @Override
        public OLD_ScrollableContainer create(final int windowId, final PlayerInventory playerInventory, @Nullable final PacketBuffer data)
        {
            if (data != null)
            {
                final int inventorySize = data.readInt();
                final BlockPos pos = data.readBlockPos();
                return new OLD_ScrollableContainer(windowId, pos, new Inventory(inventorySize), playerInventory.player, null);
            }
            return null;
        }
    }
