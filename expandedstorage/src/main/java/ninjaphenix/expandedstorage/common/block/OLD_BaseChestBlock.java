...
public abstract class OLD_BaseChestBlock<T extends OLD_AbstractChestTileEntity> extends ContainerBlock
{
            ...
            return Optional.of(new IDataNamedContainerProvider()
            {
                ...
                public Container createMenu(final int windowId, final PlayerInventory playerInventory, final PlayerEntity player)
                {
                    if (first.canOpen(player) && second.canOpen(player))
                    {
                        ...
                        return OLD_Networker.INSTANCE.getContainer(windowId, first.getPos(), inventory, player, getDisplayName());
                    }
                    return null;
                }
            });
            ...
            return Optional.of(new IDataNamedContainerProvider()
            {
                ...
                public Container createMenu(final int windowId, final PlayerInventory playerInventory, final PlayerEntity player)
                {
                    if (single.canOpen(player))
                    {
                        ...
                        return OLD_Networker.INSTANCE.getContainer(windowId, single.getPos(), single, player, getDisplayName());
                    }
                    return null;
                }
            });
    ...
    protected OLD_BaseChestBlock(final Properties builder, final Supplier<TileEntityType<? extends T>> tileEntityType)
    {
        super(builder);
            ...
            final BlockPos pos, final boolean alwaysOpen)
    {
        ...
        return TileEntityMerger.func_226924_a_(tileEntityType.get(), OLD_BaseChestBlock::getMergeType,
                OLD_BaseChestBlock::getDirectionToAttached, FACING, state, world, pos, isChestBlocked);
    }
            ...
            final PlayerEntity player, final Hand handIn, final BlockRayTraceResult hit)
    {
        if (!world.isRemote)
        {
            ...
            {
                OLD_Networker.INSTANCE.openContainer((ServerPlayerEntity) player, provider);
            ...
            @Nullable final LivingEntity placer, final ItemStack stack)
    {
        if (stack.hasDisplayName())
        {
            ...
            if (tileEntity instanceof OLD_AbstractChestTileEntity) { ((OLD_AbstractChestTileEntity) tileEntity).setCustomName(stack.getDisplayName()); }
        }
    }
