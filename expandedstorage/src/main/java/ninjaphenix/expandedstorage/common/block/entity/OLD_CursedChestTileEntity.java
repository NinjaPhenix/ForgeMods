...
//public final class OLD_CursedChestTileEntity extends OLD_AbstractChestTileEntity implements IChestLid, ITickableTileEntity
//{
//    private float animationAngle, lastAnimationAngle;
//    private int viewerCount, ticksOpen;
//
//    public OLD_CursedChestTileEntity() { this(null); }
//
//    public OLD_CursedChestTileEntity(@Nullable final ResourceLocation block) { super(OLD_ModContent.CURSED_CHEST_TE, block); }
//
//    private static int tickViewerCount(final World world, final OLD_CursedChestTileEntity instance, final int ticksOpen, final int x,
...
//                        .filter(player -> player.openContainer instanceof OLD_AbstractContainer)
//                        .map(player -> ((OLD_AbstractContainer<?>) player.openContainer).getInv())
...
//        if (OLD_BaseChestBlock.getMergeType(state) == TileEntityMerger.Type.SECOND) { return; }
//        final Vector3i offset = OLD_BaseChestBlock.getDirectionToAttached(getBlockState()).getDirectionVec();
...
//        if (block instanceof OLD_CursedChestBlock)
//        {
